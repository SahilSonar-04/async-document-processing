"""Google Gemini LLM client and embedding service integration.

This module provides:
- Structured metadata extraction with JSON schema validation and auto-repair prompts.
- Dense text embedding generation via `gemini-embedding-2` with batching.
- Single-document Retrieval-Augmented Generation (RAG) question answering.
- Usage tracking (prompt, completion, and total token consumption).
"""
...
class LLMExtractionError(RuntimeError):
    """Raised when structured field extraction via LLM fails or returns unparseable output."""
    pass


class LLMServiceError(RuntimeError):
    """Raised when LLM API communication, embedding generation, or question answering fails."""
    pass


@dataclass
class LLMExtraction:
    """Container for extracted document fields and diagnostic execution metadata.

    Attributes:
        fields: Normalized dictionary of extracted fields (title, category, summary, keywords).
        metadata: Execution diagnostics (latency, token usage, truncation, repair status).
    """
    fields: dict[str, Any]
...
class EmbeddingBatch:
    """Container for dense vector embeddings and API usage statistics.

    Attributes:
        vectors: List of 768-dimensional float embedding vectors matching input text order.
        metadata: Batch processing diagnostics (model, dimensions, input count, latency, tokens).
    """
...
def _parse_llm_json(raw: str) -> dict[str, Any]:
    """Strip markdown formatting fences and parse raw string into a JSON dictionary.
    ...
    Raises:
        ValueError: If parsed result is not a dictionary.
...
def _usage_metadata(response: Any) -> dict[str, int | None]:
    """Extract token consumption metrics from a Google GenAI API response.

    Args:
        response: Response object from Google GenAI client.

    Returns:
        dict[str, int | None]: Dictionary containing prompt, completion, and total token counts.
    """
...
def _normalise_fields(parsed: dict[str, Any], baseline: dict[str, Any]) -> dict[str, Any]:
    """Sanitize and merge LLM-extracted fields with baseline classical heuristics.

    Ensures that empty or missing fields fall back to classical extraction results
    and enforces maximum keyword constraints and valid category enums.

    Args:
        parsed: Raw dictionary extracted from LLM JSON response.
        baseline: Dictionary of fallback values generated by classical heuristics.

    Returns:
        dict[str, Any]: Normalized document metadata dictionary.
    """
...
async def extract_fields_llm(
    ...
    """Extract structured document metadata using Google Gemini with schema validation.

    Sends document text to the configured Gemini model, enforces strict JSON schema output,
    and applies automated repair prompting if initial response parsing fails.

    Args:
        text: Extracted plain text content of the document.
        filename: Original filename of the document for additional context.
        baseline: Classical heuristic fields used as fallbacks for missing values.

    Returns:
        LLMExtraction: Normalized fields and execution performance metadata.

    Raises:
        LLMExtractionError: If API credentials are missing, API call fails, or JSON is unparseable.
    """
    ...
    try:
        ...
        async def generate(contents: str) -> str:
...
def _get_gemini_client() -> Any:
    ...
    Returns:
        genai.Client: Configured Google GenAI SDK client.

    Raises:
        LLMServiceError: If provider is invalid, API key is missing, or client init fails.
    """
    if settings.llm_provider.lower() != "gemini":
...
async def embed_texts(texts: list[str], prefix: str) -> EmbeddingBatch:
    ...
    Args:
        ...
        prefix: Context prefix prepended to each text (e.g. "Document passage:" or "Search query:").

    Returns:
        EmbeddingBatch: Collection of 768-dimensional float vectors and latency/token metadata.

    Raises:
        LLMServiceError: If embedding generation fails or dimensions mismatch configuration.
    """
    if not texts:
...
async def embed_document_chunks(chunks: list[str]) -> EmbeddingBatch:
    """Generate vector embeddings for all document chunks in batched API requests.

    Args:
        chunks: List of document passage strings.

    Returns:
        EmbeddingBatch: Complete list of embeddings and aggregated batch metadata.
    """
    vectors: list[list[float]] = []
    metadata: dict[str, int | str] = {
        "provider": "gemini",
...
async def embed_question(question: str) -> list[float]:
    """Generate a 768-dimensional embedding vector for a search query or question.

    Args:
        question: Natural language question or search query string.

    Returns:
        list[float]: 768-dimensional float embedding vector.
    """
...
async def answer_document_question(question: str, excerpts: list[str]) -> str:
    """Synthesize a factual plain-text answer using retrieved document passage excerpts.

    Args:
        question: User's natural language question.
        excerpts: Ordered list of relevant text excerpts retrieved via semantic search.

    Returns:
        str: Factual answer strictly grounded in the provided document excerpts.

    Raises:
        LLMServiceError: If no excerpts are provided, API call fails, or model returns empty text.
    """
    if not excerpts:
